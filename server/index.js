...
app.post(
  ...
  wrap(async (req, res) => {
    ...
    const user = await store.read((s) => s.users.find((u) => u.email === data.email.toLowerCase()));
...
app.put(
  ...
  wrap(async (req, res) => {
    const body = z
      .object({
        settings: z.record(z.string(), z.union([z.string().max(20000), z.number()])),
    ...
    res.json(
      await orgWrite(req, (o) => {
        ...
        for (const k of body.clearSecrets || []) if (secretFields.includes(k)) delete o.secrets[k];
...
async function work() {
  ...
  try {
    ...
    try {
      ...
      const result = await integration.publishInstagram(s, task.a.promotion, async (container) => {
        await store.transact((db) => {
          const p = activity(
